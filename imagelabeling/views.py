...
from .models import ImageLabel, MachineLearningModel, MachineLearningNumbersModel, ModelSchema, FieldSchema, NumberLabel,  DigitFeature, AlphaInput, ModelEvaluation
...
def retrainModel(ml_model_id):
    # get path to CSV that has the model data
    ml_model = get_object_or_404(MachineLearningModel, pk=ml_model_id)
...
def filter(request, ml_model_id):
    ...
    for elem in clean_uncertain_list:
        elem_set = ImageLabel.objects.only('id').filter(title__contains=elem)
        for elem in elem_set:
            id_tuple.append(elem.id)
...
# this should be replaced with the ridgetuning view
